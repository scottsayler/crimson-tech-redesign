...
export const restaurantResearch: Research[] = [
  ...
  },
  {
    ...
    content: [
      ...
      `Impact cascade:`,
      `• Something breaks at the store`,
      `• Remote diagnosis stalls on unfamiliar gear`,
      `• Lunch keeps moving`,
      `• A truck roll gets scheduled`,
      `• The rush gets wasted`,
      ...
      `Question: What is restaurant networking work?`,
      `Answer: Defining and governing a reference network architecture for every store — segmentation, backup path role, remote access, equipment tiers, and format templates — so the fleet is supportable and scalable. Not carrier shopping, outage response scripts, or monitoring tool selection.`,
      `Question: How is this different from restaurant technology standardization?`,
      `Answer: Standardization owns what every store type should include at the category level. Restaurant networking owns how the network layer is structured to deliver those requirements.`,
      `Question: When should a restaurant group consider SD-WAN?`,
      `Answer: After a reference architecture exists — and when footprint size, WAN inconsistency, and remote policy needs outgrow what consistent edge standards and simpler redundancy can govern. Smaller groups usually benefit from segmentation and tested backup first.`,
      `Question: How does networking relate to PCI compliance?`,
      `Answer: Flat networks where guest Wi‑Fi reaches POS segments expand PCI scope and audit risk. Segmentation is an architecture decision with compliance consequences, not a compliance manual.`,
      `Question: What should we fix first?`,
      `Answer: Survey what is installed, publish a reference architecture, enforce it at the next opening, and validate that backup carries revenue traffic under load. Carrier replacements and SD-WAN come after that foundation.`,
    ],
    relatedSolutions: ["technology-advisory","connectivity-infrastructure"],
    relatedIndustries: ["restaurants"],
  },
