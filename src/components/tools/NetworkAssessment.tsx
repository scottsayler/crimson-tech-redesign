...
function SectionStepper({
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3" role="navigation" aria-label="Assessment sections">
...
export function NetworkAssessment({ definition, onComplete }: NetworkAssessmentProps) {
  ...
  const sortedCategoryScores = useMemo(
    () => [...results.categoryScores].sort((a, b) => a.score - b.score),
    [results.categoryScores]
  );

  const weakestCategoryIds = useMemo(
  ...
  if (showIntro && !showResults && Object.keys(answers).length === 0) {
    return (
      <AssessmentIntro
        title="Network Readiness Assessment"
        subtitle="Evaluate the resilience, standardization, and operational readiness of your multi-location network."
  ...
  if (showResults) {
    return (
      <AssessmentResultsReport
        ...
        weakestCategoryIds={weakestCategoryIds}
        onReviewAnswers={() => {
          setShowResults(false);
